...
const FacultyStudentReports: React.FC = () => {
  ...
  const loadReports = async (testType?: string, subject?: string) => {
    try {
      ...
      const uniqueBatches = [...new Set((data as StudentReport[]).map((r: StudentReport) => r.batch).filter(Boolean))];
      const uniqueBranches = [...new Set((data as StudentReport[]).map((r: StudentReport) => r.branch).filter(Boolean))];
      const uniqueSections = [...new Set((data as StudentReport[]).map((r: StudentReport) => r.section).filter(Boolean))];

      setBatches(uniqueBatches as string[]);
      setBranches(uniqueBranches as string[]);
      setSections(uniqueSections as string[]);

      const allReports = (await apiService.getFacultyStudentReports()) as StudentReport[];
