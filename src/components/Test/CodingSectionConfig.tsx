...
const CodingSectionConfig: React.FC<CodingSectionConfigProps> = ({
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      // First get the list of coding questions
      // First get the list of coding questions
      const response = await api.get<{data: any[]} | any[]>('/coding/questions');
      const questions = Array.isArray(response) ? response : (response.data || []);
      
      // Then fetch full details for each question
      const questionsWithDetails = await Promise.all(
        questions.map(async (q: {id?: string; _id?: string}) => {
          const questionId = q.id || q._id;
          const detailsResponse = await api.get<{data: CodingQuestion}>(`/coding/questions/${questionId}`);
          ...
          return {
            ...
            memory_limit: details.memory_limit || 512,
            time_limit: details.time_limit || 2000,
            supported_languages: details.supported_languages || ['python', 'javascript'],
            tags: details.tags || [],
            // Keep any existing points and timeLimit from selectedQuestions
            points: selectedQuestions.find(sq => sq.id === q.id)?.points || 100,
            timeLimit: selectedQuestions.find(sq => sq.id === q.id)?.timeLimit || 3600
          };
        })
      );

      setAvailableQuestions(questionsWithDetails);
    } catch (error) {
  ...
  const addQuestion = (question: CodingQuestion) => {
    if (!question._id && !question.id) return;
  ...
  const updatePoints = (questionId: string | undefined, points: number) => {
    if (!questionId) return;
    onQuestionsChange(
  ...
  const updateTimeLimit = (questionId: string | undefined, timeLimit: number) => {
    if (!questionId) return;
    onQuestionsChange(
