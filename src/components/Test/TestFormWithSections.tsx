...
const TestFormWithSections: React.FC<TestFormWithSectionsProps> = ({ onSubmit, loading, initialData }) => {
  ...
  const [showCodingQuestionsPreview, setShowCodingQuestionsPreview] = useState(false);
  ...
  const defaultTopicOptions: Record<string, string[]> = {
  ...
  const [topicOptionsState, setTopicOptionsState] = React.useState<Record<string, string[]>>(defaultTopicOptions);
  const [newTopicInput, setNewTopicInput] = React.useState('');

  // Load topics from server for selected subject (if available)
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        const res: any = await apiService.getSubjectTopics(formData.subject);
        if (mounted && res && res.topics) {
          setTopicOptionsState(prev => ({ ...prev, [formData.subject]: Array.from(new Set([...(prev[formData.subject] || []), ...res.topics])) }));
        }
      } catch (e) {
  ...
  const validateForm = (): { isValid: boolean; errors: any } => {
    ...
    // Handle validation for coding-only practice test
    if (formData.testType === 'Practice' && formData.hasCodingSection) {
      ...
      // Skip all MCQ validations for coding-only practice tests
      if (!formData.testName.trim()) newErrors.testName = 'Test name is required';
      if (!formData.testDescription.trim()) newErrors.testDescription = 'Description is required';
      if (!formData.startDateTime) newErrors.startDateTime = 'Start date is required';
      if (!formData.endDateTime) newErrors.endDateTime = 'End date is required';
    ...
    try {
      const payload = {
        ...
        codingQuestions: formData.hasCodingSection ? (formData.codingQuestions || []).map(q => ({
          ...
          time_limit: q.timeLimit || q.time_limit || 3600,
          memory_limit: q.memory_limit || 512,
          supported_languages: q.supported_languages || ['python', 'javascript'],
          tags: q.tags || [],
          points: q.points || 100,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="md:col-span-3">
            ...
            <div className="mt-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Topics (based on selected subject)</label>
                ...
                {(topicOptionsState[formData.subject] || []).map((topic) => (
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (!val) return;
                    try {
                      const res: any = await apiService.createSubjectTopic(formData.subject, val);
                      // update local topic options
                      setTopicOptionsState(prev => ({ ...prev, [formData.subject]: res.topics }));
                      // auto-select the newly added topic
                      handleTopicChange(val, true);
                      setNewTopicInput('');
                    } catch (err: any) {
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  // Reset any existing errors when toggling coding section
                  setErrors({});
                }}
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
      ...
      {formData.hasCodingSection && (
        <div className="bg-white p-6 rounded-lg border border-green-300 bg-green-50">
          ...
          <div className="flex justify-between items-start mb-4">
            <CodingSectionConfig
              selectedQuestions={(formData.codingQuestions || []).map(q => ({
                ...q,
                description: q.description || '',
                constraints: q.constraints || '',
                input_format: q.input_format || '',
            ...
            />
            {formData.codingQuestions && formData.codingQuestions.length > 0 && (
              <button
                type="button"
                onClick={() => setShowCodingQuestionsPreview(true)}
      ...
      {!formData.hasSections && !shouldShowSectionOption(formData.testType) && !(formData.testType === 'Practice' && formData.hasCodingSection) && (
        <div className="bg-white p-6 rounded-lg border">
      ...
      {showCodingQuestionsPreview && formData.codingQuestions && (
        <CodingQuestionPreviewModal
          isOpen={true}
          onClose={() => setShowCodingQuestionsPreview(false)}
          questions={formData.codingQuestions.map(q => ({
            _id: q.id,
            title: q.title,
            description: '', // These fields need to be fetched from the server
            difficulty: q.difficulty,
            constraints: '',
            input_format: '',
            output_format: '',
