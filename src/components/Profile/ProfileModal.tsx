...
// Define User type locally if '../../types/User' does not exist
export type User = {
  id: string;
  name: string;
  email: string;
    ...
    const fetchUserData = async () => {
      try {
        ...
        setProfileData({
          ...
          branch: typedUserData.branch || '',
          batch: typedUserData.batch || '',
          section: typedUserData.section || '',
          companyName: typedUserData.companyName || '',
          companyAddress: typedUserData.companyAddress || ''
        });
      } catch (error) {
  ...
  return (
    <div className="space-y-6 relative">
      <button
        type="button"
        onClick={onClose}
        className="absolute top-0 right-0 mt-2 mr-2 text-gray-400 hover:text-gray-600 text-2xl font-bold focus:outline-none"
        aria-label="Close"
      >
        ×
      </button>
