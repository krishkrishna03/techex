...
import {  FileText, Clock, Play, CheckCircle, XCircle, TrendingUp, Award, Target, Activity } from 'lucide-react';
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ activeTab }) => {
  ...
  const loadPerformanceData = async () => {
    try {
      ...
      setPerformanceData(reports as any[]);
    } catch (error) {
  ...
  const loadAssignedTests = async (testType?: string) => {
    try {
      ...
      const allTests = await apiService.getStudentAssignedTests() as AssignedTest[];
  ...
  const handleStartTest = async (testId: string) => {
    ...
    try {
      ...
      const response = await apiService.startTest(testId) as { test?: any; startTime?: string };
  ...
  if (activeTest && testStartTime) {
    console.log('Rendering TCS-style test interface');
