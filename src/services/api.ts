export const API_BASE_URL = import.meta.env.VITE_API_URL || 'https://college-sync-hub-tan.vercel.app/api';
class ApiService {
  ...
  async getActivitySummary() {
    return this.request('/admin/metrics/activity-summary');
  }

  async uploadPlatformLogo(formData: FormData) {
    const token = localStorage.getItem('token');
  ...
  async updateThemeSettings(themeData: { primaryColor?: string; secondaryColor?: string }) {
  ...
  async exportPlatformData(timeRange = '30') {
    return this.request(`/settings/export?timeRange=${encodeURIComponent(timeRange)}`);
  }
  ...
  // Subjects and topics
  async getSubjectTopics(subjectName: string) {
    return this.request(`/subjects/${encodeURIComponent(subjectName)}/topics`);
  }

  async createSubjectTopic(subjectName: string, topic: string) {
    return this.request(`/subjects/${encodeURIComponent(subjectName)}/topics`, {
      method: 'POST',
